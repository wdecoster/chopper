...
fn parse_gc_value(s: &str) -> Result<f64, String> {
    let f: f64 = s.parse().map_err(|e| format!("Invalid number: {e}"))?;
...
struct Cli {
    ...
    /// Minimum number of consecutive bases below --cutoff required to split a read.
    /// Shorter low-quality runs are tolerated and kept inside the segment.
    /// Only used with the `split-by-low-quality` trimming approach.
    #[arg(
        long,
        value_parser,
        default_value_t = 1,
...
/// Warns (on stderr) about trimming arguments that were set to a non-default value
/// but are ignored under the selected trimming approach, or because no trimming
/// approach was selected at all. These are warnings, not errors: the program continues.
fn warn_irrelevant_trim_args(args: &Cli) {
    let cutoff_used = matches!(
        args.trim_approach,
        Some(TrimApproach::TrimByQuality)
            | Some(TrimApproach::BestReadSegment)
            | Some(TrimApproach::SplitByLowQuality)
    );
    let crop_used = matches!(args.trim_approach, Some(TrimApproach::FixedCrop));
    let split_window_used = matches!(args.trim_approach, Some(TrimApproach::SplitByLowQuality));

    let warn = |flag: &str| {
        if args.trim_approach.is_some() {
            eprintln!("Warning: {flag} is set but is ignored by the selected trimming approach.");
        } else {
            eprintln!(
                "Warning: {flag} is set but is ignored because no --trim-approach was selected."
            );
        }
    };

    if args.cutoff.is_some() && !cutoff_used {
        warn("--cutoff");
    }
    if args.headcrop != 0 && !crop_used {
        warn("--headcrop");
    }
    if args.tailcrop != 0 && !crop_used {
        warn("--tailcrop");
    }
    if args.split_window != 1 && !split_window_used {
        warn("--split-window");
    }
}
...
fn build_trimming_approach(args: &Cli) -> Option<Arc<dyn TrimStrategy>> {
    warn_irrelevant_trim_args(args);
    if let Some(trim_approach) = args.trim_approach {
        match trim_approach {
            TrimApproach::FixedCrop => {
                ...
                Some(Arc::new(FixedCropStrategy::new(
                    args.headcrop,
                    args.tailcrop,
                )))
            }
            ...
            TrimApproach::BestReadSegment => {
                if let Some(cutoff) = args.cutoff {
                    Some(Arc::new(HighestQualityTrimStrategy::new(
                        phred_score_to_probability(cutoff),
                    )))
                } else {
                    eprintln!(
            ...
            TrimApproach::SplitByLowQuality => {
                if let Some(cutoff) = args.cutoff {
                    Some(Arc::new(SplitByLowQualityStrategy::new(
                        cutoff,
                        args.minlength,
                        args.split_window,
                    )))
...
fn sequential_filter<T>(
    input: &mut T,
    args: &Cli,
    aligner_option: &Option<Arc<Aligner<Built>>>,
    trimmer_strategy: &Option<Arc<dyn TrimStrategy + 'static>>,
) where
    T: Read + std::marker::Send,
{
...
mod tests {
    ...
    fn test_too_short_record_filtered_before_trimming() {
        ...
        assert!(
            result.is_empty(),
            "Expected record to be filtered out due to minlength"
        );
    }
    ...
    fn test_valid_before_and_after_trimming() {
        ...
        assert_eq!(
            result,
            vec![(0, 10)],
            "Record should remain valid after trimming"
        );
    }
    ...
    fn test_quals() {
        ...
        assert_eq!(
            quals,
            &vec![
                17, 19, 11, 5, 3, 19, 22, 24, 20, 22, 30, 31, 32, 20, 21, 30, 28, 10, 13, 12, 18,
                18, 18, 19, 24, 25, 35, 33, 34, 35, 34, 27, 29, 25, 21, 18, 19, 12, 14, 15, 24, 26,
                24, 7, 12, 17, 17, 19, 17, 8, 14, 15, 13, 15, 9, 3, 4, 23, 23, 29, 23, 10, 29, 30,
                31, 27, 25, 14, 2, 13, 19, 14, 13, 13, 3, 2, 10, 17, 19, 25, 27, 20, 19, 11, 5, 7,
                8, 8, 5, 2, 10, 12, 16, 18, 16, 14, 12, 15, 2, 3
            ],
